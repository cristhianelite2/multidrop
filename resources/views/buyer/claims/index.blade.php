...
@section('subheading', 'Reporta un problema con un pedido')

@section('content')
@include('partials.platform-contact', [
    'title' => 'Contacto de soporte',
    'intro' => 'Si prefieres escribirnos directamente (además de abrir un reclamo aquí):',
    'boxClass' => 'buyer-card',
    'boxStyle' => 'margin-bottom:16px',
])

<div class="buyer-card">
    <h2 style="margin:0 0 12px;font-size:1.05rem">Nuevo reclamo</h2>
    <form method="post" action="{{ route('buyer.claims.store') }}" style="display:grid;gap:12px;max-width:520px">
        @csrf
        <div>
    ...
    <table style="width:100%;border-collapse:collapse;font-size:14px">
        ...
        @forelse($claims as $claim)
            <tr style="border-top:1px solid #e2e8f0">
                ...
                <td style="padding:12px 16px">{{ $claim->order?->number }} · {{ $claim->store?->name }}</td>
                <td style="padding:12px 16px">{{ $claim->statusLabel() }}</td>
                <td style="padding:12px 16px"><a href="{{ route('buyer.claims.show', $claim) }}">Ver</a></td>
            </tr>
        @empty
